 ...
 * @author wdani
 */
public class BurbujaAscendente implements Runnable {
    ...
    public BurbujaAscendente(GraficaOrdenada pantalla, String datos, String titulos, String nombre, int velocidad, JLabel pasos, Thread t2, JPanel grafico, JLabel tiempo, JLabel vel, JLabel orden) {
        this.datos = datos;
    ...
    public void run() {
            ...
            for (int j = 0; j < datosFinal.length - 1; j++) {
                if (Integer.parseInt(datosFinal[j][1]) > Integer.parseInt(datosFinal[j + 1][1])) {
                    ...
                    try {
                        ...
                        try {
                            OutputStream crearImagen = new FileOutputStream("Imagenes/GraficaBurbujaAscendente.png") {
                            };
        ...
        //datoMinimo[0][0]= datosFinal[0][0];
        //datoMinimo[0][1]= datosFinal[0][1];

        //datoMaximo[0][0]= datosFinal[datosFinal.length][0];
        //datoMaximo[0][1]= datosFinal[datosFinal.length][1];
        System.out.println(datosFinal[0][0] + " " + datosFinal[0][1]);
        System.out.println(datosFinal[datosFinal.length - 1][0] + " " + datosFinal[datosFinal.length - 1][1]);
        t2.stop();
        JOptionPane.showMessageDialog(pantalla, "Ordenamiento terminado", "Completado", JOptionPane.INFORMATION_MESSAGE);
        generarReporte("Reporte Ordenamiento Burbuja Ascendente", "ReporteBurbujaAscendente", datosFinal, datosInicial, titulosFinal);
    }
    ...
    private void generarReporte(String titulo, String ruta, String ultimos[][], String inicio[][], String titulos[]) {
        ...
        String html = "<html>\n"
                ...
                + datosFinal
                + "</tr></table>\n"
                + "<center><img src=\"../Imagenes/GraficaBurbujaAscendente.png\" width=\"80%\" height=\"600px\"></center>"
                + "\n</body>\n"
                + "</html>";
