 ...
 * @author wdani
 */
public class QuickSortDescendente implements Runnable {
    ...
    public QuickSortDescendente(GraficaOrdenada pantalla, String datos, String titulos, String nombre, int velocidad, JLabel pasos, Thread t2, JPanel grafico, JLabel tiempo, JLabel vel, JLabel orden) {
        this.datos = datos;
    ...
    public void run() {
        ...
        quickSort(datosFinal, 0, datosFinal.length - 1, pasos, titulosFinal);
        t2.stop();
        JOptionPane.showMessageDialog(pantalla, "Ordenamiento terminado", "Completado", JOptionPane.INFORMATION_MESSAGE);
        generarReporte("Reporte QuickSort Descendente", "ReporteQuickDescendente", datosFinal, datosInicial, titulosFinal);
    }
    ...
    private void generarReporte(String titulo, String ruta, String ultimos[][], String inicio[][], String titulos[]) {
        ...
        String html = "<html>\n"
                ...
                + datosFinal
                + "</tr></table>\n"
                + "<center><img src=\"../Imagenes/GraficaQuickDescendente.png\" width=\"80%\" height=\"600px\"></center>"
                + "\n</body>\n"
                + "</html>";
    ...
    public void quickSort(String datosFinal[][], int izquierda, int derecha, int pasos, String titulosFinal[]) {
        int pivote = Integer.parseInt(datosFinal[izquierda][1]);
        String pivote2 = datosFinal[izquierda][0];
        int i = izquierda;
        int j = derecha;
        int cambio;
        String cambio2;
        ...
        while (i < j) {
            while (Integer.parseInt(datosFinal[i][1]) >= pivote && i < j) {
                i++;
            }
            while (Integer.parseInt(datosFinal[j][1]) < pivote) {
                j--;
            }
            if (i < j) {
                cambio = Integer.parseInt(datosFinal[i][1]);
                cambio2 = datosFinal[i][0];
                datosFinal[i][1] = datosFinal[j][1];
                datosFinal[i][0] = datosFinal[j][0];
                datosFinal[j][1] = String.valueOf(cambio);
                datosFinal[j][0] = cambio2;

            }
        }

        datosFinal[izquierda][0] = datosFinal[j][0];
        datosFinal[izquierda][1] = datosFinal[j][1];
        datosFinal[j][1] = String.valueOf(pivote);
        datosFinal[j][0] = pivote2;
        ...
        try {
            ...
            try {
                OutputStream crearImagen = new FileOutputStream("Imagenes/GraficaQuickDescendente.png") {
                };
        ...
        if (izquierda < j - 1) {
            quickSort(datosFinal, izquierda, j - 1, pasos, titulosFinal);
        }
        if (j + 1 < derecha) {

            quickSort(datosFinal, j + 1, derecha, pasos, titulosFinal);
        }
    }
}
