 ...
 * @author wdani
 */
public class ShellSortDescendente implements Runnable {
    ...
    public ShellSortDescendente(GraficaOrdenada pantalla, String datos, String titulos, String nombre, int velocidad, JLabel pasos, Thread t2, JPanel grafico, JLabel tiempo, JLabel vel, JLabel orden) {
        this.datos = datos;
    ...
    public void run() {
        ...
        shellSort(datosFinal, pasos, titulosFinal);
        t2.stop();
        JOptionPane.showMessageDialog(pantalla, "Ordenamiento terminado", "Completado", JOptionPane.INFORMATION_MESSAGE);
        generarReporte("Reporte ShellSort Descendente", "ReporteShellDescendente", datosFinal, datosInicial, titulosFinal);
    ...
    private void generarReporte(String titulo, String ruta, String ultimos[][], String inicio[][], String titulos[]) {
        ...
        String html = "<html>\n"
                ...
                + datosFinal
                + "</tr></table>\n"
                + "<center><img src=\"../Imagenes/GraficaShellDescendente.png\" width=\"80%\" height=\"600px\"></center>"
                + "\n</body>\n"
                + "</html>";
    ...
    public void shellSort(String datosFinal[][], int pasos, String titulosFinal[]) {
        ...
        for (salto = datosFinal.length / 2; salto != 0; salto /= 2) {
            cambios = true;
            while (cambios) {
                cambios = false;
                for (i = salto; i < datosFinal.length; i++) {
                    if (Integer.parseInt(datosFinal[i - salto][1]) < Integer.parseInt(datosFinal[i][1])) {
                        cambio = Integer.parseInt(datosFinal[i][1]);
                        cambio2 = datosFinal[i][0];
                        datosFinal[i][0] = datosFinal[i - salto][0];
                        datosFinal[i][1] = datosFinal[i - salto][1];
                        datosFinal[i - salto][0] = cambio2;
                        datosFinal[i - salto][1] = String.valueOf(cambio);
                        cambios = true;
                        this.pasos++;
                        ...
                        try {
                            ...
                            try {
                                OutputStream crearImagen = new FileOutputStream("Imagenes/GraficaShellDescendente.png") {
                                };
